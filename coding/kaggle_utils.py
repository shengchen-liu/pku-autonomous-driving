...
def draw_predict_result_label(image, truth_mask, truth_label, probability_label, stack='horizontal', scale=-1):
    ...
    for c in range(4):
        draw_shadow_text(overlay, 'pos%d %0.2f (%d)' % (c + 1, probability_label[c], truth_label[c]), (5, (c + 1) * 24),
                         0.75, color[c + 1], 1)
...
def run_check_rle():
    ...
    image = cv2.imread(os.path.join(config.train_data, '002fc4e19.jpg'), cv2.IMREAD_COLOR)
    value = [
        '002fc4e19.jpg_1',
...
def run_make_split():
    ...
    np.save(os.path.join(config.data_dir, split, 'train0_%d.npy' % len(train)), train)
    np.save(os.path.join(config.data_dir, split, 'train0_%d.npy' % len(valid)), valid)
...
def run_make_test_split():
    df = pd.read_csv(os.path.join(config.data_dir, 'sample_submission.csv'))
    df['ImageId'] = df['ImageId_ClassId'].apply(lambda x: x.split('_')[0])
    uid = df['ImageId'].unique().tolist()

    test = ['test_images/' + i for i in uid]
    np.save(os.path.join(config.data_dir, 'split/test_%d.npy' % len(test)), test)
...
def run_make_test_split1():
    ...
    np.save(os.path.join('split/test_train_%d.npy' % len(valid)), valid)
    np.save(os.path.join('split/test_valid_%d.npy' % len(train)), train)
...
def run_make_dummy():
    ...
    for i in image_id:
        ...
        image = cv2.imread(os.path.join(config.train_data, '%s' % (i)), cv2.IMREAD_COLOR)
        mask = np.array([run_length_decode(r, height=256, width=1600, fill_value=1) for r in rle])
        ...
        for k in range(2):
            ...
            dump_dir = os.path.join(config.data_dir, 'dump')
